...
func NewCreateCmd(f *cmdutil.Factory, runF func(*CreateOptions) error) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: heredoc.Doc(`
			# Create a new API key for the appID "my-app" targeting the index "MOVIES", with the "search" and "browse" ACL and a description
			$ algolia apikeys create --indices MOVIES --acl search,browse --description "Search & Browse API Key"

			# Create a new API key for the appID "my-app" targeting the indices "MOVIES" and "SERIES", with the "https://example.com" referer, with a validity of 1 hour and a description
...
func runCreateCmd(opts *CreateOptions) error {
	...
	res, err := client.CreateAPIKey(opts.AppID, key)
	if err != nil {
		return err
	}
