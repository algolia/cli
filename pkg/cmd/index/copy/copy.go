...
func NewCopyCmd(f *cmdutil.Factory, runF func(*CopyOptions) error) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:               "copy <source-index> <destination-index> [--scope <scope-1>...] [--wait]",
		...
		`),
		Example: heredoc.Doc(`
			Copy the records, settings, synonyms and rules from TEST_PRODUCTS_1 to TEST_PRODUCTS_2
			$ algolia index copy TEST_PRODUCTS DEV_PRODUCTS

			Copy only the synonyms of TEST_PRODUCTS_1 to TEST_PRODUCTS_2
			$ algolia index copy TEST_PRODUCTS DEV_PRODUCTS --scope synonyms

			Copy the synonyms and rules of TEST_PRODUCTS_1 to TEST_PRODUCTS_2
