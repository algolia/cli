...
func runImportCmd(opts *ImportOptions) error {
	...
	for opts.Scanner.Scan() {
		...
		// If requested, only clear existing rules for the first batch (otherwise we'll keep deleting added rules)
		clearExistingRules := false
		if count == 1 {
			clearExistingRules = opts.ClearExistingRules
		}

		// Technically, SaveRules already batches, but this manual setup prevents having to read and parse everything first, and only then index it.
		if count == batchSize {
