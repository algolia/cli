...
func runSelectCmd(opts *SelectOptions) error {
	...
	// If a profile already exists for this app, just switch the default.
	...
	// No profile yet — fetch full details and create one.
	opts.IO.StartProgressIndicatorWithLabel("Fetching application details")
	appDetails, err := client.GetApplication(accessToken, chosen.ID)
	opts.IO.StopProgressIndicator()
	if err != nil {
		return fmt.Errorf("failed to fetch application details: %w", err)
	}

	return apputil.ConfigureProfile(opts.IO, opts.Config, appDetails, "", true)
}
