...
func ConfiguredProfilesCompletionFunc(
	f *Factory,
	...
	return func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
		...
		for _, profile := range profiles {
			completions = append(
				completions,
				fmt.Sprintf("%s\t%s", profile.Name, profile.ApplicationID),
			)
		}
...
func StringSliceCompletionFunc(
	allowedMap map[string]string,
...
func runStringSliceCompletion(
	...
	for name, description := range allowedMap {
		...
		if !utils.Contains(prefixSlice, name) &&
			strings.HasPrefix(strings.ToLower(name), toComplete) {
			...
			results = append(results, fmt.Sprintf(
				...
				fmt.Sprintf(
					"%s %s",
					prefixAllDescription,
					utils.SliceToReadableString(dynamicSliceDescriptions),
				),
			))
		}
	}
